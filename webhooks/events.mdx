...
| `customer.balance.updated` | A customer's store credit balance was changed (credit, debit, purchase, or refund) |
...
Fired when a customer's store credit balance is changed. This includes manual adjustments via the dashboard, API adjustments, purchase deductions, and refund credits.

**Common use cases:**
- Sync customer balance to an external loyalty or rewards system
- Send a notification when credit is added to a customer's account
- Log balance changes for accounting and auditing
- Trigger workflows when a customer receives a refund credit
...
```json
{
  ...
  "event_id": "f2a3b4c5-d6e7-8901-abcd-012345678901",
  "data": {
    "customer_id": 123,
    "transaction": {
      "id": 456,
      ...
      "description": "Store credit for loyalty reward"
    }
  }
}
```
...
<Note>
  The `source` field indicates what triggered the balance change. Possible values: `manual_adjustment`, `purchase`, `refund`, `promotion`, `api`, `purchase_cancelled`, `reservation`.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="customer.balance.updated">
    Use when you need to sync balance changes to external loyalty, rewards, or accounting systems.
  </Card>
</CardGroup>
